...
public class MiniumStoneClient implements ClientModConstructor {
    ...
    public void onRegisterItemDecorations(ItemDecorationContext context) {
        context.registerItemDecorator(
                (GuiGraphics guiGraphics, Font font, ItemStack stack, int itemPosX, int itemPosY) -> {
                    int charge = MiniumStoneItem.getCharge(stack) + 1;
                    int startX = itemPosX + 2;
                    int startY = itemPosY + 13;
                    guiGraphics.pose().pushPose();
                    for (int i = 0; i < charge; i++) {
                        guiGraphics.fill(RenderType.guiOverlay(), startX + 1 + 3 * i, startY + 1,
                                startX + 1 + 3 * i + 2, startY + 2, ChatFormatting.AQUA.getColor() | 0xFF000000
                        );
                    }
